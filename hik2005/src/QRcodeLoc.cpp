...
public:
    ...
    bool find_add(CameraFrame frame, QRcodeInfo *info)
    {
        ...
        {
            // log_file << "can not identify code:" << frame.code << std::endl;
            stream.str("");
...
public:
    ...
    QRcodeLoc(){
        ...
        QRmap_tab = new QRcodeTable(cfg_dir+"QRmap_table.txt");
        Lidarmap_tab = new QRcodeTable(cfg_dir+"Lidarmap_table.txt");
        ...
        // // 日志文件初始化
        // std::string log_name = log_dir + "/LocError" + format_time(ros::Time::now()) + ".txt";
        // log_err.open(log_name);
        // if (!log_err.is_open())
        // {
        ...
        // }
        // ros::Subscriber sub = nh.subscribe<nav_msgs::Odometry>("/hik2005/odom_map",1, &QRcodeLoc::err_callback, this);
    ...
    void err_callback(const nav_msgs::Odometry::ConstPtr& msg)
    {
        tf2_ros::Buffer buffer;
        tf2_ros::TransformListener tfListener(buffer);
        geometry_msgs::TransformStamped trans_base2map;
