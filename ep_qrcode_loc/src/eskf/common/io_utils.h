...
class TxtIO {
   public:
    ...
    using MagProcessFuncType = std::function<void(const Mag &)>;
    using IICProcessFuncType = std::function<void(const IIC &)>;
    ...
    TxtIO &SetMagProcessFunc(MagProcessFuncType mag_proc) {
        mag_proc_ = std::move(mag_proc);
        return *this;
    }

    TxtIO &SetIICProcessFunc(IICProcessFuncType iic_proc) {
        iic_proc_ = std::move(iic_proc);
        return *this;
    }
   ...
   private:
    ...
    MagProcessFuncType mag_proc_;
    IICProcessFuncType iic_proc_;
