...
void TxtIO::Go() {
    ...
    while (!fin.eof()) {
        ...
        if (data_type == "IMU" && imu_proc_) {
            ...
            ss >> time >> ax >> ay >> az >> gx >> gy >> gz;
            imu_proc_(IMU(time/1000000, Vec3d(gx, gy, gz), Vec3d(ax, ay, az)));
        } else if (data_type == "ODOM" && odom_proc_) {
            double time, vx, vy, vz, gx, gy, gz;
            ss >> time >> vx >> vy >> vz >> gx >> gy >> gz;
            //vx = vx*0.8;
            odom_proc_(Odom(time/1000000, 0.0, 0.0, vx, gz));
        } else if (data_type == "MAG" && mag_proc_) {
            double time, w, x, y, z;
            ss >> time >> w >> x >> y >> z;
            mag_proc_(Mag(time/1000000, Quatd(w, x, y, z)));
        } else if (data_type == "IIC" && iic_proc_) {
            double time, px, py, pz, ow, ox, oy, oz;
            ss >> time >> px >> py >> pz >> ow >> ox >> oy >> oz;
            iic_proc_(IIC(time/1000000, Vec3d(px, py, pz), Quatd(ow, ox, oy, oz)));
