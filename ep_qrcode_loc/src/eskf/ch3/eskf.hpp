...
#include "common/mag.h"
#include "common/iic.h"
...
class ESKF {
   public:
    ...
    using MagNoiseT = Eigen::Matrix<S, 3, 3>;       // 磁力计噪声类型
    using IicNoiseT = Eigen::Matrix<S, 6, 6>;       // IIC噪声类型
    ...
    struct Options {
        ...
        /// 磁力计参数
        double mag_var_ = 1e-5;

        /// IIC参数
        double iic_var_ = 1e-5;
        double iic_pos_noise_ = 10.0;                   // IIC位置噪声
        double iic_ang_noise_ = 20.0 * math::kDEG2RAD;  // IIC旋转噪声
    ...
    /// 使用磁力计观测
    bool ObserveMagnetometer(const Mag& mag);

    /// 使用lio-SAM观测
    bool ObserveImuIncremental(const IIC& iic);
   ...
   private:
    void BuildNoise(const Options& options) {
        ...
        // 设置磁力计噪声
        double m2 = options_.mag_var_ * options_.mag_var_;
        mag_noise_.diagonal() << m2, m2, m2;

        // 设置IIC噪声
        double i2 = options_.iic_var_ * options_.iic_var_;
        mag_noise_.diagonal() << i2, i2, i2;
    ...
    MagNoiseT  mag_noise_  = MagNoiseT::Zero();
    IicNoiseT  iic_noise_  = IicNoiseT::Zero();
    GnssNoiseT gnss_noise_ = GnssNoiseT::Zero();

    /// 标志位
    bool first_gnss_ = true;  // 是否为第一个gnss数据
    bool first_mag_ = true;  // 是否为第一个mag数据
    bool first_iic_ = true;  // 是否为第一个iic数据
...
bool ESKF<S>::ObserveWheelSpeed(const Odom& odom) {
    ...
    VecT vel_odom(odom.vx_, 0.0, 0.0);
...
template <typename S>
bool ESKF<S>::ObserveMagnetometer(const Mag& mag) {
    assert(mag.timestamp_ >= current_time_);

    if (first_mag_) {
        R_ = mag.pose_;
        first_mag_ = false;
        current_time_ = mag.timestamp_;
        return true;
    }

    // mag 修正以及雅可比
    // 使用三维的观测，H为3x18，大部分为零
    Eigen::Matrix<S, 3, 18> H = Eigen::Matrix<S, 3, 18>::Zero();
    H.template block<3, 3>(0, 6) = Mat3T::Identity();

    // 卡尔曼增益
    Eigen::Matrix<S, 18, 3> K = cov_ * H.transpose() * (H * cov_ * H.transpose() + mag_noise_).inverse();

    // update x和cov
    dx_ = K * (R_.inverse() * mag.pose_).log();
    cov_ = (Mat18T::Identity() - K * H) * cov_;
...
template <typename S>
bool ESKF<S>::ObserveImuIncremental(const IIC& iic) {
    assert(iic.timestamp_ >= current_time_);

    if (first_iic_) {
        R_ = iic.pose_.so3();
        p_ = iic.pose_.translation();
        first_iic_ = false;
        current_time_ = iic.timestamp_;
        return true;
    }

    ObserveSE3(iic.pose_, options_.iic_pos_noise_, options_.iic_ang_noise_);
    current_time_ = iic.timestamp_;
